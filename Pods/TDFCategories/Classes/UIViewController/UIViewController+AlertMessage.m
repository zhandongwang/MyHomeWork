...
#define TDFCategoryLocalizedString(key, comment) \
NSLocalizedStringFromTableInBundle(key, @"TDFCategoryLocalizable", [NSBundle bundleForClass:NSClassFromString(@"TDFCGFindResourceVC")], comment)
...
- (UIAlertController *)showTipAlertWithMessage:(NSString *)message {
    UIAlertController *avc = [UIAlertController alertControllerWithTitle:TDFCategoryLocalizedString(@"提示", nil)
                                                                 message:message
...
- (void)showMessage:(NSString *)message confirmTitle:(NSString *)confirmTitle cancelTitle:(NSString *)cancelTitle confirm:(void(^)(void))confirmBlock cancel:(void (^)(void))cancelBlock {
    [self showAlertWithTitle:TDFCategoryLocalizedString(@"提示", nil) message:message confirmTitle:confirmTitle cancelTitle:cancelTitle confirm:confirmBlock cancel:cancelBlock];
}
...
- (void)showMessage:(NSString *)message confirm:(void(^)(void))confirmBlock cancel:(void (^)(void))cancelBlock {
    ...
    [self showMessage:message buttonTitle:TDFCategoryLocalizedString(@"确认", nil) confirm:confirmBlock cancel:cancelBlock];
}

- (void)showAlert:(NSString *)message confirm:(void(^)(void))code {

    [self showAlert:message buttonTitle:TDFCategoryLocalizedString(@"我知道了", nil) confirm:code];
}
...
- (void)showMessage:(NSString *)message buttonTitle:(NSString *)title confirm:(void (^)(void))confirmBlock cancel:(void (^)(void))cancelBlock {
    ...
    UIAlertController *avc = [UIAlertController alertControllerWithTitle:TDFCategoryLocalizedString(@"提示", nil)
                                                                 message:message
                                                          preferredStyle:UIAlertControllerStyleAlert];
    [avc addAction:[UIAlertAction actionWithTitle:TDFCategoryLocalizedString(@"取消", nil) style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
